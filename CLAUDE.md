...
```bash
# 依存関係インストール + pre-commit フック設定（初回のみ）
make setup
...
make setup         # 依存インストール + pre-commit フック設定（初回のみ）
make lint-all      # pre-commit を全ファイルに実行
```
...
詳細な手順・オプション一覧は **[docs/operations-runbook.md](docs/operations-runbook.md)** を参照。
...
| `deactivate_user.py` | ユーザーを停止（`is_activated: False`） | `PROJECT_ID=clearbag-dev uv run python scripts/deactivate_user.py --email user@example.com` |
