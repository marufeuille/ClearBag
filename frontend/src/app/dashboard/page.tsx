...
function DashboardContent() {
  ...
  // 共有ファイルのアップロード成功時にドキュメント一覧を更新
  useEffect(() => {
    if (shareTarget.documentId) {
      setSuccessMsg("共有ファイルを解析キューに登録しました。数分後に結果が表示されます。");
      setErrorMsg(null);
      setRefreshKey((k) => k + 1);
      const timer = setTimeout(() => setSuccessMsg(null), 5000);
      return () => clearTimeout(timer);
    }
  }, [shareTarget.documentId]);

  // 共有エラー時にエラーメッセージを表示
  useEffect(() => {
    if (shareTarget.error) {
      setErrorMsg(shareTarget.error);
      setSuccessMsg(null);
    }
  }, [shareTarget.error]);
