...
from v2.adapters.firestore_repository import StoredTaskData
from v2.domain.models import DocumentRecord, EventData
...
_CREATED_AT = datetime.datetime(2025, 10, 25, 9, 0, 0, tzinfo=datetime.UTC)

_DEFAULT_RECORD = DocumentRecord(
    id=_DOC_ID,
    uid=_UID,
    status="completed",
    content_hash=_HASH,
...
class TestGetDocument:
    ...
    def test_get_document_includes_created_at(self, client):
        """created_at が ISO 8601 形式でレスポンスに含まれること"""
        response = client.get(f"/api/documents/{_DOC_ID}")
        assert response.status_code == 200
        data = response.json()
        assert data["created_at"] == _CREATED_AT.isoformat()

    def test_get_document_created_at_none_when_missing(self, client, mock_doc_repo):
        """created_at が None のとき null を返すこと"""
        mock_doc_repo.get.return_value = DocumentRecord(
            id=_DOC_ID,
            uid=_UID,
            status="completed",
            content_hash=_HASH,
...
class TestGetDocumentDetail:
    """GET /api/documents/{id}/detail のテスト"""

    def test_detail_returns_empty_events_and_tasks(self, client):
        """イベント・タスクなしの場合は空リストを返す"""
    ...
    def test_detail_returns_events_and_tasks(self, client, mock_doc_repo):
        """イベント・タスクが存在する場合はそれぞれのリストを返す"""
        mock_doc_repo.list_events_by_document.return_value = [
            EventData(
                summary="遠足",
                start="2025-10-25",
                end="2025-10-25",
        ...
        mock_doc_repo.list_tasks_by_document.return_value = [
            StoredTaskData(
                id="task-1",
                title="同意書の提出",
                due_date="2025-10-10",
        ...
        assert data["events"][0]["summary"] == "遠足"
        assert data["events"][0]["start"] == "2025-10-25"

        assert len(data["tasks"]) == 1
        assert data["tasks"][0]["title"] == "同意書の提出"
        assert data["tasks"][0]["completed"] is False

    def test_detail_nonexistent_document_returns_404(self, client, mock_doc_repo):
        """存在しないドキュメントは 404 を返す"""
        mock_doc_repo.get.return_value = None
...
class TestGetDocumentUrl:
    """GET /api/documents/{id}/url のテスト"""

    def test_url_returns_signed_url(self, client, mock_storage):
        """署名付き URL が返ること"""
        mock_storage.generate_signed_url.return_value = (
            "https://storage.googleapis.com/signed"
        )

        response = client.get(f"/api/documents/{_DOC_ID}/url")
        assert response.status_code == 200
        data = response.json()
        assert data["url"] == "https://storage.googleapis.com/signed"
        assert data["mime_type"] == "application/pdf"
        assert data["filename"] == "20251025_遠足_長男.pdf"  # archive_filename を使う

    def test_url_falls_back_to_original_filename(
        self, client, mock_doc_repo, mock_storage
    ):
        """archive_filename が空のとき original_filename を使う"""
        mock_doc_repo.get.return_value = DocumentRecord(
            id=_DOC_ID,
            uid=_UID,
            status="completed",
    ...
    def test_url_nonexistent_document_returns_404(self, client, mock_doc_repo):
        """存在しないドキュメントは 404 を返す"""
        mock_doc_repo.get.return_value = None
        response = client.get("/api/documents/nonexistent/url")
        assert response.status_code == 404
